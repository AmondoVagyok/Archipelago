"""All locations tied to Prison Breakout, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - PRISON_BREAKOUT_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - PRISON_BREAKOUT_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - PRISON_BREAKOUT_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - PRISON_BREAKOUT_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - PRISON_BREAKOUT_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
PRISON_BREAKOUT_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    PRISON_BREAKOUT_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
PRISON_BREAKOUT_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

PRISON_BREAKOUT_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

PRISON_BREAKOUT_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
PRISON_BREAKOUT_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    PRISON_BREAKOUT_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
