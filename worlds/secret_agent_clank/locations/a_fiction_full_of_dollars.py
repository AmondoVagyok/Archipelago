"""All locations tied to A Fiction Full Of Dollars, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - A_FICTION_FULL_OF_DOLLARS_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - A_FICTION_FULL_OF_DOLLARS_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - A_FICTION_FULL_OF_DOLLARS_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - A_FICTION_FULL_OF_DOLLARS_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - A_FICTION_FULL_OF_DOLLARS_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
A_FICTION_FULL_OF_DOLLARS_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    A_FICTION_FULL_OF_DOLLARS_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
A_FICTION_FULL_OF_DOLLARS_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

A_FICTION_FULL_OF_DOLLARS_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

A_FICTION_FULL_OF_DOLLARS_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
A_FICTION_FULL_OF_DOLLARS_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    A_FICTION_FULL_OF_DOLLARS_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
