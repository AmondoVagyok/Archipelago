"""All locations tied to Dams Edge Hydrano, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - DAMS_EDGE_HYDRANO_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - DAMS_EDGE_HYDRANO_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - DAMS_EDGE_HYDRANO_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - DAMS_EDGE_HYDRANO_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - DAMS_EDGE_HYDRANO_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
DAMS_EDGE_HYDRANO_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    DAMS_EDGE_HYDRANO_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
DAMS_EDGE_HYDRANO_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"Mission: {_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

DAMS_EDGE_HYDRANO_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

DAMS_EDGE_HYDRANO_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
DAMS_EDGE_HYDRANO_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    DAMS_EDGE_HYDRANO_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
