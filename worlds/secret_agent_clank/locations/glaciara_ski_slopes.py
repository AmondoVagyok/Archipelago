"""All locations tied to Glaciara Ski Slopes, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - GLACIARA_SKI_SLOPES_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - GLACIARA_SKI_SLOPES_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - GLACIARA_SKI_SLOPES_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - GLACIARA_SKI_SLOPES_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - GLACIARA_SKI_SLOPES_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
GLACIARA_SKI_SLOPES_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    GLACIARA_SKI_SLOPES_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
GLACIARA_SKI_SLOPES_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

GLACIARA_SKI_SLOPES_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

GLACIARA_SKI_SLOPES_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
GLACIARA_SKI_SLOPES_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    GLACIARA_SKI_SLOPES_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
