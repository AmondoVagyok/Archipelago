"""All locations tied to Saint Qwark, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - SAINT_QWARK_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - SAINT_QWARK_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - SAINT_QWARK_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - SAINT_QWARK_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - SAINT_QWARK_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
SAINT_QWARK_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    SAINT_QWARK_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
SAINT_QWARK_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

SAINT_QWARK_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

SAINT_QWARK_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
SAINT_QWARK_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    SAINT_QWARK_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
