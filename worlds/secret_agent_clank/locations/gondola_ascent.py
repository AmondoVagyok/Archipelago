"""All locations tied to Gondola Ascent, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - GONDOLA_ASCENT_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - GONDOLA_ASCENT_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - GONDOLA_ASCENT_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - GONDOLA_ASCENT_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - GONDOLA_ASCENT_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
GONDOLA_ASCENT_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    GONDOLA_ASCENT_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
GONDOLA_ASCENT_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

GONDOLA_ASCENT_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

GONDOLA_ASCENT_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
GONDOLA_ASCENT_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    GONDOLA_ASCENT_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
