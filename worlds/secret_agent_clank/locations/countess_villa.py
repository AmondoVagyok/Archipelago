"""All locations tied to Countess Villa, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - COUNTESS_VILLA_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - COUNTESS_VILLA_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - COUNTESS_VILLA_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - COUNTESS_VILLA_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - COUNTESS_VILLA_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
COUNTESS_VILLA_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    COUNTESS_VILLA_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
COUNTESS_VILLA_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"Mission: {_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

COUNTESS_VILLA_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

COUNTESS_VILLA_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
COUNTESS_VILLA_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    COUNTESS_VILLA_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
