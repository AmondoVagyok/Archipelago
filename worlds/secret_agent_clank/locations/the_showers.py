"""All locations tied to The Showers, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - THE_SHOWERS_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - THE_SHOWERS_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - THE_SHOWERS_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - THE_SHOWERS_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - THE_SHOWERS_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
THE_SHOWERS_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    THE_SHOWERS_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
THE_SHOWERS_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

THE_SHOWERS_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

THE_SHOWERS_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
THE_SHOWERS_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    THE_SHOWERS_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
