"""All locations tied to Boltaire Gem Wing, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - BOLTAIRE_GEM_WING_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - BOLTAIRE_GEM_WING_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - BOLTAIRE_GEM_WING_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - BOLTAIRE_GEM_WING_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - BOLTAIRE_GEM_WING_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
BOLTAIRE_GEM_WING_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    BOLTAIRE_GEM_WING_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
BOLTAIRE_GEM_WING_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

BOLTAIRE_GEM_WING_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

BOLTAIRE_GEM_WING_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
BOLTAIRE_GEM_WING_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    BOLTAIRE_GEM_WING_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
