"""All locations tied to Venantonio Labs, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - VENANTONIO_LABS_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - VENANTONIO_LABS_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - VENANTONIO_LABS_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - VENANTONIO_LABS_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - VENANTONIO_LABS_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
VENANTONIO_LABS_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    VENANTONIO_LABS_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
...
VENANTONIO_LABS_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

VENANTONIO_LABS_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

VENANTONIO_LABS_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
VENANTONIO_LABS_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    VENANTONIO_LABS_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
