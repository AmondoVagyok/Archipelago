"""All locations tied to Galactic Bolt Reserve, grouped into 4 dicts by which
...
locations/__init__.py itself.
  ...
  - GALACTIC_BOLT_RESERVE_LOCATIONS: always created (weapon vendor / gadget pickup /
    gadgetbot / special / ratchet challenge entries for this case).
  - GALACTIC_BOLT_RESERVE_MISSION_LOCATIONS: level_completion granularity (default), gated by Missions.
  - GALACTIC_BOLT_RESERVE_ALL_MISSIONS_LOCATIONS: all granularity, gated by Missions.
  - GALACTIC_BOLT_RESERVE_CUTSCENE_LOCATIONS: gated by AllCutscenes.
  - GALACTIC_BOLT_RESERVE_OTHER_LOCATIONS: this case's skill points + keycards + alien
...
GALACTIC_BOLT_RESERVE_LOCATIONS: dict[str, SACLocationData] = {}
for _name in WEAPONS_BY_CASE.get(_CASE_NAME, ()):
    GALACTIC_BOLT_RESERVE_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
for _name in (
...
GALACTIC_BOLT_RESERVE_MISSION_LOCATIONS: dict[str, SACLocationData] = {
    f"{_CASE_NAME} Complete": SACLocationData(_take_id(), _CASE_NAME),
}

GALACTIC_BOLT_RESERVE_ALL_MISSIONS_LOCATIONS: dict[str, SACLocationData] = all_mission_locations(_CASE_NAME, _take_id)

GALACTIC_BOLT_RESERVE_CUTSCENE_LOCATIONS: dict[str, SACLocationData] = {
...
GALACTIC_BOLT_RESERVE_OTHER_LOCATIONS: dict[str, SACLocationData] = {}
for _name in (
    *SKILL_POINTS_BY_CASE.get(_CASE_NAME, ()),
    *KEYCARDS_BY_CASE.get(_CASE_NAME, ()),
    *ALIEN_CODES_BY_CASE.get(_CASE_NAME, ()),
):
    GALACTIC_BOLT_RESERVE_OTHER_LOCATIONS[_name] = SACLocationData(_take_id(), _CASE_NAME)
