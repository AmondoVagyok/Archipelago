...
class LocationHookTests(unittest.TestCase):
    ...
    def test_preinit_entitlements_preserve_unmanaged_slots_and_return_address(self):
        program = words(entitlement_routine(0x180000, 0x200000, 0x350000))
        regs = [0] * 32
        regs[31] = 0x123456
        mem = Memory()
    ...
    def test_code_and_marker_are_word_aligned(self):
        ...
        for record in (False, True):
            self.assertEqual(len(flag_routine(0x180000, 0x350000, record=record)) % 4, 0)
        with self.assertRaises(ValueError):
            jump(0x350001)

    def test_managed_getter_uses_location_flag(self):
        for flag, expected in ((1, 0), (2, 1)):
            target, regs, memory = run_routine(flag_routine(0x180000, 0x350000, record=False), 17, flag)
    ...
    def test_native_grant_latches_location_and_preserves_arguments(self):
        target, regs, memory = run_routine(flag_routine(0x180000, 0x350000, record=True), 26, 1)
    ...
    def test_unmanaged_and_out_of_range_ids_use_original_function(self):
        for record in (False, True):
            for gadget, flag in ((17, 0), (40, 1), (0xFFFFFFFF, 1)):
                target, regs, _ = run_routine(flag_routine(0x180000, 0x350000, record=record), gadget, flag)
