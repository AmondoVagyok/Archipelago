"""Max-Security Cells's per-location rules -- every location belonging to
...
case at all is already gated by its "To Max-Security Cells" entrance (see
...
def set_max_security_cells_rules(world: "SecretAgentClankWorld") -> None:
    ...
    world.set_rule(mw.get_location(SACRatchetWeapons.RATCHETPDA, player), True_())
    world.set_rule(mw.get_location(SACRatchetWeapons.BOLTTRANSFER, player), True_())
