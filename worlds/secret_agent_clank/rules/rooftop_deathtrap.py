"""Rooftop Deathtrap's per-location rules -- every location belonging to
...
case at all is already gated by its "To Rooftop Deathtrap" entrance (see
...
def set_rooftop_deathtrap_rules(world: "SecretAgentClankWorld") -> None:
    ...
    world.set_rule(mw.get_location(SACClankGadgets.CUFFLINK, player), True_())
    world.set_rule(mw.get_location(SACRatchetWeapons.OMNIKEY, player), True_())
