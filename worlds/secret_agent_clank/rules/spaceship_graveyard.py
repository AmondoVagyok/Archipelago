...
from .rule_helpers import Has, HasGadget, HasProjectileWeapon
...
def set_spaceship_graveyard_rules(world: "SecretAgentClankWorld") -> None:
    player = world.player
    mw = world.multiworld
    _base = HasAll(SACClankGadgets.JETBOOTS, HasProjectileWeapon())
    _omnikey = _base & HasGadget(SACClankGadgets.OMNIKEY)
    _tangle = _omnikey & HasGadget(SACClankWeapons.TANGLEVINE)
