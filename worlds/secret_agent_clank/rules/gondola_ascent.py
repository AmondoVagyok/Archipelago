"""Gondola Ascent's per-location rules -- every location belonging to
...
case at all is already gated by its "To Gondola Ascent" entrance (see
...
def set_gondola_ascent_rules(world: "SecretAgentClankWorld") -> None:
    ...
    # Always-on
    world.set_rule(mw.get_location(SACTitaniumBoltLocations.GONDOLA_ASCENT_1, player), True_())

    # Story mission (Missions)
    if world.options.all_missions.value == Missions.option_all:
        world.set_rule(mw.get_location(SACMissionLocations.GONDOLA_ASCENT_GET_A_LIFT, player), True_())
    else:
        world.set_rule(mw.get_location(SACMissionLocations.GONDOLA_ASCENT_COMPLETE, player), True_())

    # Cutscene (AllCutscenes)
    if world.options.all_cutscenes:
        world.set_rule(mw.get_location(SACCutsceneLocations.GONDOLA_ASCENT_FINISH_GONDOLA_CUTSCENE, player), True_())

    # Skill point (SkillPoints)
    if world.options.skill_points:
        world.set_rule(mw.get_location(SACSkillPointLocations.GONDOLA_ASCENT_STEEL_RAIN, player), True_())
    ...
    if world.options.all_alien_codes:
        ...
        world.set_rule(
            mw.get_location(SACAlienCodeLocations.GONDOLA_ASCENT_CARLS_SECRET, player), HasGadget(THERM_OPTIC_SHADES),
        )
        world.set_rule(
            mw.get_location(SACAlienCodeLocations.GONDOLA_ASCENT_JESS_SECRET, player), HasGadget(THERM_OPTIC_SHADES),
        )
