...
class Progression:
    ...
    def prepare(self, symbols, hooks, module, *, vendor_enabled=True):
        ...
        give, buy = symbols.get('WeaponPickup_GiveWeapon__FP4Moby'), symbols.get('SCRNVENDOR_ProcessPurchase__Fv')
        if give is not None:
        ...
        def allocate(data):
            ...
            if not extra_storage:
                ...
                guards, extra_ranges = prepare_gain_storage(p, symbols)
                edits.extend(guards)
        ...
        if self.ng_plus and vendor_enabled:
            from .titan_vendor import prepare_titan_price
            edits.extend(prepare_titan_price(p, symbols, allocate))
        targets = [('PLAYER_GrantBolts__Fi', self.bolts, 4, 0x27BDFFF0, 0xFFB00000)]
        ...
        if module != 31:
            targets.append(('GLOBALVARS_AddExperience__Fi', self.health_xp, 4, 0x27BDFFE0, None))
            if not self.enabled:
                targets.append(('GADGET_GetsXP__FUiUib', self.weapon_xp, 5, 0x27BDFFB0, 0xFFB30028))
        for name, multiplier, register, first, second in targets:
            ...
            address = allocate(gain_wrapper(target, original, register, multiplier))
