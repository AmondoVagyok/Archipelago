...
def set_inside_clank_rules(world: "RACSizeMatterWorld") -> None:
    ...
    _base = HasAll(Rac5Gadgets.HYPERSHOT, Rac5Gadgets.POLARIZER) & HasShrinkRayDoorAccess(world)
    ...
    if world.options.challenge_mode.value >= 1:
        if weapon_enabled(world, Rac5Weapons.STATIC_BARRIER):
            world.set_rule(
                mw.get_location(Rac5TitanVendorLocations.INSIDE_CLANK_STATIC_TITAN, player),
                _base & HasChallengeMode(world, 1),
            )
    if world.options.challenge_mode.value >= 2:
        world.set_rule(
            mw.get_location(Rac5Locations.INSIDE_CLANK_CHAMELEON_HELMET, player), HasChallengeMode(world, 2)
        )
